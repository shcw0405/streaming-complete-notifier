...
本仓库也包含 [回响 Relay 桌面应用](apps/relay/README.md)，支持 macOS Apple Silicon。浏览器扩展和桌面端可分别安装、独立通知；连接后可统一管理网页与终端工具的提醒设置。

- 桌面版 0.3 新增「SSH 提醒」配置向导：为远端 Codex 配置本地 VS Code 终端响铃，附逐步测试和可复制命令。
- 桌面端默认驻留顶部菜单栏，不占用 Dock，不自动弹窗；点击菜单栏「打开回响」进入管理窗口。
- 管理界面使用网页技术，既能在独立应用窗口中运行，也能通过本机地址在浏览器中打开，不是云端网站。
- 首次连接保留双方原设置；连接后公共声音、音量、桌面通知设置支持双向同步。扩展写入采用确认机制，通常 30 秒内同步；桌面界面每 5 秒刷新。
- 当前支持一个浏览器配置，离线期间可独立使用；本机离线修改后可用「同步本机提醒偏好到扩展」统一设置。

安装与连接步骤见 [试用说明](apps/relay/INSTALL.md)。
