...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    Menu.setApplicationMenu(Menu.buildFromTemplate([
      { label: '回响 Relay', submenu: [{ label: '关于回响', click: () => dialog.showMessageBox(window, { message: '回响 Relay 0.3.1 试用版', detail: '网页 AI 与本地编程工具的统一提醒入口。关闭窗口后仍在菜单栏运行；退出后已接入的工具仍可独立通知。' }) }, { type: 'separator' }, { role: 'quit', label: '退出回响' }] },
